...
export function evaluateEventTruth({
  ...
  const isTrustedStructuredSource =
    verification.structuredCardSource === true &&
    TRUSTED_STRUCTURED_SOURCES.has(normalizeText(candidate.sourcePrimary));
  if (compatibleSourceCount < 2 && !isTrustedStructuredSource) {
