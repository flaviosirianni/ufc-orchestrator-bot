...
| UFC-STAB-002 | alta | DONE | UFC-STAB-001 | Aplicar quality pack, documentar símbolos nuevos y asociar tests/evidencia runtime. | `check-quality-pack.sh` sin bloqueos. | `qualityPack.test.js`, `npm run quality:gate` y fixture Git/worktree en verde; hooks versionados configurados vía `core.hooksPath=.githooks`. |
