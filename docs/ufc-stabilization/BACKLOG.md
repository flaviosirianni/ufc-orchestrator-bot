...
| ID | Prioridad | Estado | Dependencias | Acción y diseño | Aceptación y pruebas | Evidencia / próximo paso |
| --- | --- | --- | --- | --- | --- | --- |
| UFC-STAB-100 | crítica | VERIFYING | Etapa 0 | Introducir `EventTruthGate`; separar estado deportivo de confianza y persistir evidencia/reasons. | Eventos `invalid/stale/degraded` no son consumibles; tests UFC 329 y fragmentos `Preview`. | RED/GREEN y suite completa pasaron. Esquema aditivo, defaults legacy `invalid`, auditoría append-only y discovery single-source no consumible. Pendiente deploy conjunto de contención. |
| UFC-STAB-101 | crítica | VERIFYING | UFC-STAB-100 | Aplicar gate a proyecciones, scoring, news, mirrors y auto-settlement; exigir stats frescas. | Stats vencidas no escriben snapshots ni cierran apuestas; la apuesta abierta permanece igual. | RED/GREEN y suite completa pasaron. Cero I/O mutante bloqueado, caminos frescos operativos, ledger SQLite real sin cambios y vista Telegram oculta mirrors contaminados. Pendiente deploy conjunto. |
| UFC-STAB-102 | crítica | VERIFYING | UFC-STAB-100 | Hash determinístico e índices de dedupe para projection/scoring. | Fake clock 24 h inserta sólo snapshot inicial y cambios reales. | RED: 96 ciclos generaban 96+96 filas. GREEN: un solo snapshot por contenido, segundo sólo ante cambio material; migración legacy aditiva conserva filas sin hash y la suite completa pasa. Pendiente deploy conjunto. |
| UFC-STAB-103 | alta | VERIFYING | UFC-STAB-100 | Ampliar `/health` con evento, stats, Odds, billing y mantenimiento sin secretos. | Campos aditivos, timestamps y reasons verificables; tests del contrato HTTP. | RED/GREEN, suite completa, quality gate y `qa:parity:ufc` pasaron. Payload top-level compatible, getters read-only, TTL revalidado y allowlist sin tokens, paths, evidencia ni cartelera. Pendiente deploy conjunto. |
...
| UFC-STAB-105 | crítica | IN_PROGRESS | UFC-STAB-100..103 (104 rolled back, ver justificación) | Gate, push, deploy, refresh forzado y observación 24 h con rollback por kill switch. | Cero cartelera falsa, snapshots repetidos o settlements no verificados. | Ejecutando deploy de contención (Fase A del plan 2026-08-11): gates de calidad/paridad → merge ff-only a main → deploy `ufc-oci` con snapshot pre/post → smoke no mutante. |
