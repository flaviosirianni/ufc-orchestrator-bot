...
| 1. Contención | WAITING_OBSERVATION | Ningún dato deportivo dudoso produce output o mutaciones |
...
| ID | Prioridad | Estado | Dependencias | Acción y diseño | Aceptación y pruebas | Evidencia / próximo paso |
| --- | --- | --- | --- | --- | --- | --- |
| UFC-STAB-100 | crítica | WAITING_OBSERVATION | Etapa 0 | Introducir `EventTruthGate`; separar estado deportivo de confianza y persistir evidencia/reasons. | Eventos `invalid/stale/degraded` no son consumibles; tests UFC 329 y fragmentos `Preview`. | Desplegado 2026-08-12 (`5d50462`, ff-only `1323308..5d50462`). `/health` post-deploy confirma `current_event`/`next_event` con `confidence:"invalid"`, `consumer_allowed:false` y reasons (`verification_missing`, `event_confidence_invalid`, `event_consumer_not_allowed`). En observación 24h. |
| UFC-STAB-101 | crítica | WAITING_OBSERVATION | UFC-STAB-100 | Aplicar gate a proyecciones, scoring, news, mirrors y auto-settlement; exigir stats frescas. | Stats vencidas no escriben snapshots ni cierran apuestas; la apuesta abierta permanece igual. | Desplegado 2026-08-12. Evidencia productiva: ciclo inicial de `preFightAnalysis` post-restart insertó 0 filas nuevas en `fight_projection_snapshots`/`fight_bet_scoring_snapshots` (antes del deploy insertaba 4+12 por ciclo sobre la UFC 329 fantasma). Ledger sin drift: `bets=49, bet_mutations=52, credit_transactions=27, mp_processed_payments=1, ledger_summary=2` — idéntico al baseline 2026-07-20. En observación 24h. |
| UFC-STAB-102 | crítica | WAITING_OBSERVATION | UFC-STAB-100 | Hash determinístico e índices de dedupe para projection/scoring. | Fake clock 24 h inserta sólo snapshot inicial y cambios reales. | Desplegado 2026-08-12 junto con 100/101. Sin incidentes en journal post-restart. En observación 24h. |
| UFC-STAB-103 | alta | WAITING_OBSERVATION | UFC-STAB-100 | Ampliar `/health` con evento, stats, Odds, billing y mantenimiento sin secretos. | Campos aditivos, timestamps y reasons verificables; tests del contrato HTTP. | Desplegado 2026-08-12; `/health` responde 200 con los 6 bloques (`process/event_intel/ufc_stats/odds_api/billing/maintenance`) 50s post-restart. Hallazgo menor no bloqueante: `billing` reporta `status:"disabled"/reason:"billing_not_configured"` pese a que billing real funciona (`GET /topup/config` → `enabled:true` en el mismo proceso) — bug cosmético aislado a `summarizeBilling`/`getHealthStatus()`, sin impacto en cobros/creditos reales; sigue pendiente de root-cause, no bloquea esta etapa. |
...
| UFC-STAB-105 | crítica | WAITING_OBSERVATION | UFC-STAB-100..103 (104 rolled back, ver justificación) | Gate, push, deploy, refresh forzado y observación 24 h con rollback por kill switch. | Cero cartelera falsa, snapshots repetidos o settlements no verificados. | Deploy completo 2026-08-12 02:09:56 UTC: gates (`npm test`, `quality:gate`, `qa:parity:ufc`) en verde → merge ff-only `main` → snapshot pre (`/home/ubuntu/pre-deploy-snapshot-20260812.json`) → restart `bot-factory@ufc` → snapshot post (`/home/ubuntu/post-deploy-snapshot-20260812.json`) → `protected_tables` idénticos, `missing_protected_tables=[]` en ambos, ledger sin drift. Kill switch: revert de `5d50462` + `git checkout 1323308` en el server + restart. Falta cerrar la ventana de observación de 24h (checkear ~2026-08-13 02:10 UTC) antes de pasar a `DONE`. |
