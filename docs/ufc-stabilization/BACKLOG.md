...
| 2. Fuentes deportivas | READY | Stats frescas y current/next reconciliados por fuentes corroboradas |
...
| UFC-STAB-204 | crítica | READY | Etapa 1 (no depende de UFC-STAB-201) | Resolver `next_event` con Odds API como fuente primaria estructurada (`resolveNextEventFromOddsRows` agrupa filas de `odds_events_index` por ventana horaria, ancla la fecha en la pelea más temprana del cluster); Google News queda como corroboración secundaria de quorum, nunca fuente de fighters/fecha. Tercer corroborador (`upcoming_fights` de `ufc_stats.db`) queda pendiente de UFC-STAB-201/203 (stats frescas). | Tests verdes: `__tests__/oddsEventResolver.test.js`, `__tests__/eventIntel.test.js` (casos UFC 330 real con quorum→`verified`, single-source→`degraded`, fallback sin odds→camino web preexistente intacto). UFC 329 ya no es discoverable desde Odds API (query `commence_time >= now`, evento pasado queda excluido de origen). | B1/B2/B3 completos y en verde local (sesión 2026-08-12): `resolveNextEventFromOddsRows`, wiring en `discoverNextEvent`/`startEventIntelMonitor`, y ambos sitios de escritura de `current_event` en `bettingWizard.js` (`wantsLiveEventStatus` y `wantsLatestNews`/`wantsEventProjections`) ahora calculan `evaluateEventTruth` en vez de persistir sin verificación. Falta B5: gates de paridad, merge, deploy y smoke productivo. |
