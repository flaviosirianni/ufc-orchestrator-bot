...
| UFC-STAB-202 | alta | IN_PROGRESS | UFC-STAB-201 | Instalar scraper OCI y timer dos veces por día con single-flight y logs. | Dos corridas consecutivas idempotentes; timer visible y activo. | Python 3.12 + venv + Playwright + Chromium instalados en `ufc-oci` (`/home/ubuntu/apps/data-scrapper`); repo clonado y sembrado con `state.json`/`fights.json`/`upcoming.json` reales del Mac (evita rescrapear desde 2016). Confirmado: ufcstats.com NO bloquea la IP del datacenter (`networkidle` + los mismos waits que el scraper real → 784 event links, contenido completo). Timer diario (no dos veces/día, sin señal de que haga falta más frecuencia) vía `ops/systemd/ufc-data-scrapper.timer` (`OnCalendar=*-*-* 08:00:00 UTC`), `flock` para single-flight. Server tenía <1GB RAM y 0 swap — se agregó swapfile de 2GB (persistente en fstab) antes de instalar Chromium, para no arriesgar el proceso de producción por OOM. Falta activar el timer (ver UFC-STAB-203). |
| UFC-STAB-203 | crítica | IN_PROGRESS | UFC-STAB-202 | Construir candidato, validar, swap atómico, restart UFC y rollback automático. | Bot abre la nueva DB y health expone frescura <36 h. | `ops/scripts/run-ufc-data-scrapper.sh`: scrape → convierte a candidato (nunca pisa la DB live directo) → valida (`PRAGMA integrity_check`, conteo de fights no debe bajar, `event_date_iso` completo) → backup + swap atómico → restart `bot-factory@ufc` → poll de `/health` hasta 240s → si algo falla en cualquier paso, la DB live queda intacta o se hace rollback automático al backup. 3 tests de integración reales (`__tests__/ufcDataScrapper.test.js`, ejecutan el script de verdad vía `execFileSync` con Python/sudo/curl fake y SQLite real): happy path, regresión de conteo (aborta sin tocar la DB live), fallo de health post-swap (rollback real, verificado byte a byte). Falta: corrida manual en el server (no vía timer) antes de habilitar `systemctl enable --now`. |
